...
public class CallForProposalServiceImpl implements CallForProposalService {
	...
	public List<CallForProposal> getCallForProposals(ListView lv,
			CallForProposalSearchCreteria searchCreteria) {
		return callForProposalDao.getCallForProposals(lv, searchCreteria);
	}

	public void prepareListView(ListView lv,
			CallForProposalSearchCreteria searchCreteria) {
		lv.setCountRows(callForProposalDao
				.countCallForProposals(searchCreteria));
		lv.setLastPage(lv.getNumOfPages());
		lv.setNearPages(lv.getScroll());
	}

	public List<CallForProposal> getCallForProposalsOnline(
			CallForProposalSearchCreteria searchCreteria) {
		return callForProposalDao.getCallForProposalsOnline(searchCreteria);
	}

	public List<CallForProposal> getCallForProposalsOnlineSimple(String ids,
			String viewType) {
		return callForProposalDao
				.getCallForProposalsOnlineSimple(ids, viewType);
	}

	/*
	 * public List<CallForProposal>
	 * getCallForProposals(CallForProposalSearchCreteria searchCriteria,String
	 * localeId){ List<CallForProposal> localeCallForProposals = new
	 * ArrayList<CallForProposal>(); for (CallForProposal callForProposal:
	 * callForProposalDao.getCallForProposals(searchCriteria)){ if
	 * (LanguageUtils
	 * .getLanguage(callForProposal.getTitle()).getLocaleId().equals(localeId))
	 * localeCallForProposals.add(callForProposal); } return
	 * localeCallForProposals; }
	 */
	...
	public int insertAttachmentToCallForProposal(int callForProposalId,
			Attachment attachment) {
		return callForProposalDao.insertAttachmentToCallForProposal(
				callForProposalId, attachment);
	}
	...
	public List<CallForProposal> getCalendarMonthCallForProposals(
			String firstDay, String lastDay) {
		return callForProposalDao.getCalendarMonthCallForProposals(firstDay,
				lastDay);
	}
