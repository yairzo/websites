...
function CreateInfoPagesTable (){
	...
	<% for (int i=0; i< researchFieldProfile.getInfoPagesByResearchField().length; i++) {%>
		...
		+'</tr></table>'
		+'</th>'
		+'<td width="30px" onClick="toggleLineImg(\u0027' + id1 + '\u0027,\u0027' + id2 + '\u0027,' +<%=i%>+')">'
		...
		+'<td id=\"' + id1 + '\" onClick="toggleLineImg(\u0027' + id1 + '\u0027,\u0027' + id2 + '\u0027,' + <%=i%>+')"  class=\"fundingopen\" >'
		+ '</td>'
		+'<td class="" onClick="toggleLineImg(\u0027' + id1 + '\u0027,\u0027' + id2 + '\u0027,' + <%=i%>+')">&nbsp;</td>'
		+'</tr>'
...
function toggleLine(id,infoPage) {
	var fieldn = '#' + id;
	var trid='tr' + id;
	body=document.getElementById(trid);
	if (body) {
		if (body.style.display == 'none') {
			$.get("infoPageDescription.jsp?type=research&infoPage="+infoPage+"&researchFieldNum="+<%=researchFieldProfile.getResearchFieldNum()%>, function(data) { 
		   		$(fieldn).html(data);
		    });
...
function toggleLineImg(id1,id2,infoPage) {
	flag = toggleLine(id1,infoPage);
