...
function CreateFundTable(fundNum){
	...
	for (i=0; i< titles[fundNum].length; i++){
		...
		+'</tr></table>'
		+'</th>'
		+'<td width="30px" onClick="toggleLineImg(\u0027' + id1 + '\u0027,\u0027' + id2 + '\u0027,'+fundNum + ',' + i+')">'
		...
		+'<td id=\"' + id1 + '\" onClick="toggleLineImg(\u0027' + id1 + '\u0027,\u0027' + id2 + '\u0027,'+fundNum + ',' + i+')"  class=\"fundingopen\" >'
		+ '</td>'
		+'<td class="" onClick="toggleLineImg(\u0027' + id1 + '\u0027,\u0027' + id2 + '\u0027,'+fundNum + ',' + i+')">&nbsp;</td>'
		+'</tr>'
...
function toggleLine(id,fundNum,infoPage) {
	var fieldn = '#' + id;
	var trid='tr' + id;
	body=document.getElementById(trid);
	if (body) {
		if (body.style.display == 'none') {
			$.get("infoPageDescription.jsp?type=fund&param="+fundNum+"&infoPage="+infoPage, function(data) { 
		   		$(fieldn).html(data);
		    });
...
function toggleLineImg(id1,id2,fundNum,infoPage) {

	flag = toggleLine(id1,fundNum,infoPage)
