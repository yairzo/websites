									...
									<% for (int j=-1; j<12; j++){%>
										...
										<% if (calendarProfile.getInfoPagesByMonth(j).length>0){%>
											<script language="javascript1.2">
												...
												contents[<%=j%>+1] = new Array (
													<% for (int i=0; i<calendarProfile.getInfoPagesByMonth(j).length; i++){%>
														"<td onClick=\"toggleLineImg(\u0027' + id1 + '\u0027,\u0027' + id2 + '\u0027)\" dir=\"<%=calendarProfile.getInfoPagesByMonth(j)[i].isHebrew() ? "rtl" : "ltr" %>\" class=\"fundingopen\" style=\"text-align: <%=calendarProfile.getInfoPagesByMonth(j)[i].isHebrew() ? "right" : "left" %>;\"><%= calendarProfile.getInfoPagesByMonth(j)[i].getFormatedDescription()%></td>"
														<%= i<calendarProfile.getInfoPagesByMonth(j).length-1 ? "," : "" %>
													<%}%>
												);
